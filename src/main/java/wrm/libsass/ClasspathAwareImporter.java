...
import static wrm.libsass.Lookups.findLocalFile;
import static wrm.libsass.Lookups.findResource;
import static wrm.libsass.Lookups.findWebJarResource;
...
class ClasspathAwareImporter implements Importer {
  ...
  public Collection<Import> apply(String importStr, Import previous) {
    ...
    Stream<Lookup> lookups = Stream.of(() -> findLocalFile(base, addUnderscore(uri)),
        () -> findLocalFile(base, uri),
        () -> uriCss.map(u -> findLocalFile(base, u)).orElse(Optional.empty()),
        () -> findResource(addUnderscore(uri)), () -> findResource(uri),
        () -> uriCss.map(Lookups::findResource).orElse(Optional.empty()),
        () -> findResource(addUnderscore(base.resolve(uri))), () -> findResource(base.resolve(uri)),
    ...
    Optional<Lookup.Result> lookupResult = lookups.map(Lookup::run)
        .filter(Optional::isPresent).map(Optional::get)
        .findFirst();
    return lookupResult.map(r -> r.buildImport(importStr))
  ...
  private static Optional<URI> tryBuildUriCss(String importStr) {
    return importStr.endsWith(SCSS_EXT) ? Optional.empty() : Optional
        .of(URI.create(importStr + CSS_EXT));
  }
